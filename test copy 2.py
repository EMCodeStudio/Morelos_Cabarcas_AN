...
def bisection_method(a, b, c, tol=1e-6, max_iter=11):
...
def show_results(a, b, c, results):
    ...
    for i, (a_val, b_val, pm, f_pm) in enumerate(results):
        ...
        error_percent = abs((pm - results[-2][2]) / results[-2][2]) * 100 if len(results) > 1 and results[-2][2] != 0 else 0

        tree.insert(
            "", "end", values=(i + 1, a_val, f_a, pm, f_pm, b_val, f_b, error_percent)
        )
