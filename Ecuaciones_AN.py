...
def formato_metodo_cuadratico():
    try:
        ...
        if x1 is None:
            ...
            resultado_cuadratico.config(
                text=(
                    f"x1 = {x1:.2f}"
                    if x1 == x2
                    else f"x1 = {x1:.2f}, x2 = {x2:.2f}"
                ))
    except ValueError:
...
def valores_metodo_falsa_posicion(limite_xl, limite_xu, maxima_iteracion):
    ...
    while contador_iteracion < maxima_iteracion:
        ...
        results.append((limite_xl, f_xl, limite_xu, f_xu, xr, f_xr))
...
def formato_tabla_falsa_posicion(limite_a, limite_b, results):
    ...
    for i, (limite_xl, f_xl, limite_xu, f_xu, xr, f_xr) in enumerate(results):
        # Calcular el porcentaje de error usando los valores actuales y anteriores de xr
        if i > 0:
            porcentaje_error_aproximado = abs((xr - results[i - 1][4]) / xr) * 100
            print("XR", xr)
            print("XR Anterior", results[i - 1][4])
        else:
...
# Campos de Entrada de Valores de los Limites
...
# Etiquetas de los Campos de Entrada de Limites
ttk.Label(root, text="a / xl").grid(row=3, column=2, padx=5, pady=5)
ttk.Label(root, text="b / xu").grid(row=3, column=4, padx=5, pady=5)
